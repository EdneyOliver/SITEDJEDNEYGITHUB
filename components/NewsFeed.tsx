    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
      {feedbacks.map((item: any) => (
        <div 
          key={item.id} 
          className={`
            relative p-6 sm:p-8 rounded-3xl border transition-all duration-300 flex flex-col justify-between group hover:-translate-y-1
            ${item.highlight 
        ...
        >
          <div>
            <div className="flex items-center justify-between mb-5">
              ...
              <i className={`fas fa-quote-right text-lg ${item.highlight ? 'text-blue-400/60' : 'text-white/15'}`}></i>
            </div>
          ...
          <div className="flex items-center gap-3.5 pt-4 border-t border-white/5">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-sync font-bold text-xs shadow-inner shrink-0 ${item.highlight ? 'bg-gradient-to-tr from-blue-600 to-indigo-600 text-white' : 'bg-white/10 text-gray-200'}`}>
